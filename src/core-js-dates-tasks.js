...
function getTime(date) {
  ...
  return `${writeTime(date.getHours())}:${writeTime(date.getMinutes())}:${writeTime(date.getSeconds())}`;
}
...
function getCountDaysInMonth(month, year) {
  const nextMont = new Date(year, month);
  const currentMonth = new Date(nextMont - 1);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (new Date(dateEnd) - new Date(dateStart)) / ms + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  let diff = Math.ceil((date.getTime() - firstDayYear.getTime()) / msInWeek);
  diff += firstDayYear.getDay() === date.getDay() ? 1 : 0;
  return diff;
}
